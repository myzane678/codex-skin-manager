# Codex Skin Manager

Windows 上的 Codex 主题管理器。它通过本机 Chrome DevTools Protocol (CDP) 将选定主题实时应用到 Codex 界面，并提供主题库、图片建主题、托盘切换和状态诊断。

> 仅支持 Windows。目前面向 Codex 桌面版结构和 Codex++ 启动时暴露的本地调试端口；未知版本需通过实时结构探针。

## 功能

- 导入、导出、启用、停用、重命名和删除 `.codextheme` 主题包。
- 内置琥珀工作台、莓果信号、森林控制台、霓虹回路四套主题。
- 从本地图片创建主题，设置可读性、视觉焦点、布局和任务页显示方式。
- 应用运行期间实时切换主题，不需要重启 Codex。
- 使用 Codex++ 启动 Codex 时，自动检测 `127.0.0.1:9229` 并附加当前主题。
- CDP 页面重载或短暂断开后自动重连；管理器使用单实例锁，避免重复注入互相覆盖。
- Codex 更新到未知版本后使用最新适配器执行实时结构探针；探针通过即可注入，并在客户端常驻提示“未验证版本”。
- 托盘菜单可快速切换原生外观或已安装主题。

## 安装与运行

从 [Releases](https://github.com/myzane678/codex-skin-manager/releases) 下载最新的 Windows 安装程序并运行。

启动管理器后，选择一个主题即可应用。若 Codex 由 Codex++ 启动并开放本机 `9229` 端口，管理器会自动附加，不需要点击“启动主题版”。

重复启动管理器会自动唤醒已有窗口，不会再创建第二个注入进程。

## 使用图片创建主题

选择“从图片创建主题”后，建议使用主体位于一侧、另一侧留出低细节空间的横向图片。这样侧边栏、对话内容和输入区仍有足够对比度，不会被背景遮挡。

主题库旁提供了中文提示词范例，可直接用于生成适合作为 Codex 背景的图片。

## 主题与数据

- 主题包格式为 `.codextheme`，可在管理器内导入、导出。
- 管理器的主题和配置保存在 Windows 用户数据目录中，不会修改 Codex 安装目录。
- 选择“恢复默认”会停止注入、关闭代理模式并恢复管理器创建的快捷方式。

## 从源码运行

环境要求：Node.js `24.15.0` 或更高版本、npm。

```powershell
npm install
npm test
npm run typecheck
npm run lint
npm run start
```

生成 Windows 安装包：

```powershell
npm run make
```

安装包会生成在 `out/make/squirrel.windows/x64/`。

## 验证范围与限制

- 本项目只连接本机回环地址，不会向远程服务器发送主题或图片。
- CDP 接口和 Codex DOM 结构会随应用版本变化。未知版本仅在实时结构探针通过后注入；探针失败时进入兼容性降级状态。
- 主题注入依赖目标 Codex 进程已启用本地调试端口；普通启动方式不保证可以自动附加。

## 致谢与许可

部分视觉规则和 Windows DOM 集成参考了 [Codex Dream Skin](https://github.com/Fei-Away/Codex-Dream-Skin) 的 MIT 许可实现。完整第三方声明见 [NOTICE.md](NOTICE.md)。

本项目以 [MIT License](LICENSE) 发布。版本变更见下方更新日志。

## 更新日志

### [0.1.7] - 2026-07-29

- 未知 Codex 版本不再因缺少精确版本适配器而直接拒绝注入，改用最新适配器执行实时 DOM 结构探针。
- 实时探针通过后正常注入并标记为“未验证版本”；探针失败时仍保持兼容性降级并禁止注入。
- 左侧当前主题状态和“启动与代理”页新增未验证版本常驻提醒，运行时诊断同步记录兼容性状态。

### [0.1.6] - 2026-07-29

- 将输入区透明壳层规则提取到共享主题编译流程，内置主题与图片生成主题保持一致。
- 根据输入框实际的 sticky 祖先自适应标记停靠区域，不依赖固定高度、间距或底部偏移。
- 清除有对话任务页输入框后方的 Codex 原生浅色渐变，修复背景底部残留白色区域。

### [0.1.5] - 2026-07-29

- 将 Dream Skin 根样式作用域迁移到稳定的 `data-codex-skin-*` 属性，避免 Codex 重写根 class 时出现原生外观闪回。
- 保留现有运行时与样式节点，不再依赖 Codex 管理的根主题 class。

### [0.1.4] - 2026-07-29

- 主题根样式与根状态在同一 Codex renderer 内常驻，切换任务或发送消息不再卸载后重装主题。
- 路由变化仅更新运行时页态，避免 React 替换内容时出现原生外观窗口。

### [0.1.3] - 2026-07-28

- 将新任务首帧注入改为 DOMContentLoaded 与 250ms 条件轮询，避免共享渲染器等待壳层时短暂恢复原生外观。
- 早期脚本注册不可用时，在页面 load 后 250ms 回退重注入。

### [0.1.2] - 2026-07-28

- 将更新日志收录到 README，作为唯一的用户更新入口。

### [0.1.1] - 2026-07-28

- 添加 MIT `LICENSE`。
- README 补充许可证说明。
- 补充 Squirrel Windows 安装器所需的发布作者元数据。

### [0.1.0] - 2026-07-28

- 支持 Codex 主题包的导入、导出、启用、停用、重命名和删除。
- 提供四个内置主题，并支持从本地图片创建主题。
- 支持主题实时切换，不需要重启 Codex。
- 支持 Codex++ 在 `127.0.0.1:9229` 暴露 CDP 时自动附加主题。
- 支持 CDP 断线重连和管理器单实例锁，避免重复注入冲突。
- 支持托盘快捷主题切换和运行时诊断。
